...
function TimelineItem({ job, index, side }) {
  ...
  return (
    ...
    >
      <div className="group bg-brand-800 rounded-2xl border border-brand-700/50 shadow-sm
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          {job.current && (
            <span className="flex-shrink-0 inline-flex items-center gap-1 text-[0.65rem]
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400 mb-4">
          <span><i className="fas fa-calendar-alt text-[10px] mr-1.5" />{job.period}</span>
          <span><i className="fas fa-map-marker-alt text-[10px] mr-1.5" />{job.location}</span>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative tl-track">
          ...
          <div className="hidden md:flex flex-col gap-10">
            {filtered.map((job, i) => (
              <TimelineItem key={`${job.company}-${i}`} job={job} index={i} side={i % 2 === 0 ? 'left' : 'right'} />
            ))}
          </div>
          ...
          <div className="md:hidden flex flex-col gap-6 pl-6">
            ...
            {filtered.map((job, i) => (
              <SectionReveal key={`m-${job.company}-${i}`} delay={`delay-${(i % 3) + 1}`}>
                <div className="relative bg-brand-800 rounded-2xl border border-brand-700/50 shadow-sm p-5">
                  <div className="absolute -left-[26px] top-6 w-3.5 h-3.5 rounded-full
                  ...
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div>
                      ...
                      <p className="text-brand-400 text-xs font-medium">{job.company}</p>
                    </div>
                    {job.current && (
                  ...
                  <div className="flex flex-wrap gap-1">
                    {job.tags.map((t, ti) => (
                      <span key={ti} className="text-[0.62rem] text-slate-400 bg-brand-700/50
                        border border-brand-600/40 rounded-full px-2 py-0.5">{t}</span>
                    ))}
                  </div>
                </div>
              </SectionReveal>
